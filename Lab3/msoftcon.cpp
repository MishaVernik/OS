...
std::mutex mx_set_cursor_pos;          // mutex for synchronous printing;
std::mutex mx_clear_screen;
std::mutex mx_save_positions;
int saved_x, saved_y;
//--------------------------------------------------------------
void init_graphics()
   {
...
void save_positions(int x, int y){
   mx_save_positions.lock();
   saved_x = x;
   saved_y = y;
   mx_save_positions.unlock();
}

//--------------------------------------------------------------
void set_cursor_pos(int x, int y, std::string text)
   {
      mx_set_cursor_pos.lock();
...
void clear_real_time_table(){   
}

void clear_screen()
   {
       mx_clear_screen.lock();    
      #if defined _WIN32
      system("cls");
...
void draw_rectangle(int left, int top, int right, int bottom)
   ...
   for(int y=top; y<=bottom; y++)  //stack of strings
      {
      set_cursor_pos(left, y, std::string(temp));
      //_cputs(temp);
      }
   }
//--------------------------------------------------------------
void draw_circle(int xC, int yC, int radius)
   ...
   for(theta=0; theta<=pi/2; theta+=increment)  //quarter circle
      ...
      while(x <= xC+xN)          //fill two horizontal lines
         ...
         set_cursor_pos(x,   yC-yN, std::to_string(fill_char));// putch(fill_char);  //top
         set_cursor_pos(x++, yC+yN, std::to_string(fill_char));// putch(fill_char);  //bottom
         }
      }  //end for
   }
//--------------------------------------------------------------
