...
namespace Lab4.AVL
{
    public class AVLTree<T> : ICollection<T>, IList<T> 
        where T : IComparable<T>
    {
        public class AVLTreeNode : ICollection<T>, IList<T>
        {
            public AVLTreeNode(T value, AVLTree<T> tree)
            {
                this.Value = value;
                this.Level = 1;
                this.Count = 1;
            ...
            public AVLTreeNode LeftHand { get; private set; }
            public AVLTreeNode RightHand { get; private set; }
            int Level { get; set; }
            ...
            public void Add(T item)
            {
                ...
                if (compare < 0)
                    if (this.LeftHand == null)
                        ((this.LeftHand = new AVLTree<T>.AVLTreeNode(item, this.Tree)).Parent = this).Reconstruct(true);
                    else this.LeftHand.Add(item);
                else
                    if (this.RightHand == null)
                    ((this.RightHand = new AVLTree<T>.AVLTreeNode(item, this.Tree)).Parent = this).Reconstruct(true);
        ...
        public AVLTreeNode RootNode { get; private set; }


        private void Path(AVLTreeNode root, string value, List<string> way)
        {
            
            if (root == null) return;
            if (root.Value.ToString() == value)
            {
                way.Add(root.Value.ToString());
            }else
                if (String.Compare(root.Value.ToString(), value) > 0)
            {
                way.Add(root.Value.ToString());
                Path(root.LeftHand, value, way);
            }
            else
            {
                way.Add(root.Value.ToString());
                Path(root.RightHand, value, way);
            }
        }
        public List<string> FindPathToNodeFromRoot(String value)
        {
            List<string> path = new List<string>();
            Path(this.RootNode, value, path);
            return path;
        }
        public void Add(T item)
        {
            if (this.RootNode == null) this.RootNode = new AVLTree<T>.AVLTreeNode(item, this);
